...
if( !class_exists( 'wzspbb_plugin_api' ) ) {

class wzspbb_plugin_api{

   public  $url = "https://bbrion.es/woo-zoho-sync-pro";
...
public function plugin_api_setup(){
  ...
  $linkText = __('Check for updates','woo-zoho-sync-pro');
...
$auto_renew=isset($update['user']['auto_renew']) ? $update['user']['auto_renew'] : '';
  ...
  $message=$this->lic_msg= sprintf(__("Your License Key expired. %sRenew it Now%s",'woo-zoho-sync-pro'),'<a href="'.$lic_url.'" target="_blank">','</a>');
  ...
  if($_GET['vx_update_check_result'] == "update_available"){
    $msg=sprintf(__('%s Update is available','woo-zoho-sync-pro'),"<b>".$this->title.".</b>");
  }else{
   $msg=sprintf(__('%s Update is not available','woo-zoho-sync-pro'),"<b>".$this->title.".</b>");
  }
  $this->screen_msg($msg);
  }
  ...
  if(isset(self::$updates['hidden'][$key.'-'.$type]) && !empty(self::$updates['hidden'][$key.'-'.$type]) && self::$updates['hidden'][$key.'-'.$type]>$time){
  continue;
  }
  $this->notice_js=true;
  if($msg['type'] == "image"){
...
public function screen_msg( $message, $level = 'updated') {
  ...
  global $vx_addons;
if(method_exists('wzspbb_addons','addon_ver') && !empty($vx_addons)){
 $addons=array();
    foreach($vx_addons as $v){
  $addons[$v]=wzspbb_addons::addon_ver($v);
 }
...
public function add_section_wc($tabs){
    ...
    $tabs["wzspbb_license"]=__('License Key','woo-zoho-sync-pro');
    }
    return $tabs;
}
...
$this->screen_msg($msg['msg'],$msg['class']);
  ...
  <h2>
  <?php esc_html_e("License Key", 'woo-zoho-sync-pro') ?>
  </h2>
  <p><?php echo sprintf(__("If you don't have a license key, you can get from %shere%s.",'woo-zoho-sync-pro'),'<a href="'.$this->url."?vx_product=".$this->sku.'" target="_blank" title="'.__('Get License Key','woo-zoho-sync-pro').'">','</a>'); ?></p>
  <table class="form-table">
  <tr>
  <th scope="row"><label for="vx_license_key">
  <?php _e("License Key", 'woo-zoho-sync-pro'); ?>
  </label>
  </th>
  <td>
  <div style="display: table" class="vx_tr">
  <div style="display: table-cell; width: 85%;">
  <input type="password" style="width: 100%;" class="crm_text" id="vx_license_key" name="vx_lic_key" placeholder="<?php _e('Enter License Key','woo-zoho-sync-pro') ?>" value="<?php echo $lic_key ?>">
  </div><div style="display: table-cell;">
  <a href="#" style="margin: 0 0 0 10px; vertical-align: baseline; text-align: center; width: 110px" class="button vx_toggle_key ddd" id="vx_toggle_key" title="<?php _e('Toggle License Key','woo-zoho-sync-pro'); ?>">Show Key</a>
  ...
  <?php
      ...
      ?>
  <div style="color:rgb(0, 132, 0); padding-bottom: 8px; margin-top: 8px;"><i class="fa fa-check"></i> <?php echo sprintf(__("License Key is Valid - %s",'woo-zoho-sync-pro'),'<code>'.date('F d, Y h:i:s A',$lic_time).'</code>'); ?></div>
  <?php
  }
                      ?></td>
  </tr>
  <tr>
  <th colspan="2" ><input type="submit" name="<?php echo $this->id."_key"; ?>" class="button-primary" title="<?php _e('Save License Key','woo-zoho-sync-pro'); ?>" value="<?php _e("Save License Key", 'woo-zoho-sync-pro') ?>" /></th>
  </tr>
  </table>
  <?php
       if(empty($form_tag)){
