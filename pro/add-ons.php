...
if( !class_exists( 'wzspbb_addons' )):
class wzspbb_addons{
    public $plugin_update=false;
    public $version='1.0';
  public static $url='https://bbrion.es/woo-zoho-sync-pro';
...
public function setup(){
  ...
  if(isset($_GET['action']) && current_user_can( 'wzspbb_edit_addons' )){
     $plugin="";
  ...
  if(isset($_REQUEST['vx_action']) && $_REQUEST['vx_action'] == "get_addons" && current_user_can( 'wzspbb_edit_addons' )){
...
if(!empty($action) && !isset($_GET['action'])){
  ?>
   <div class="updated notice is-dismissible"><p><?php echo sprintf(__('Go to %sZoho Sync Pro%s Page to manage Add-ons','woo-zoho-sync-pro'),'<a href="'.$this->get_link('settings').'"><b>','</b></a>');?> </p>
  </div>
  <?php
...
public function main_menu(){
               $page_title =__('Zoho Sync Pro','woo-zoho-sync-pro');
        $menu_title = __('Zoho Sync Pro','woo-zoho-sync-pro');
        $capability = 'wzspbb_view_addons';
...
public function admin_menu(){
           $page_title =__('Zoho Sync Pro Add-ons','woo-zoho-sync-pro');
        $menu_title = __('Zoho Sync Pro Add-ons','woo-zoho-sync-pro');
        $capability = 'wzspbb_view_addons';
        $function   = array( $this, 'vx_addons');
...
$access=self::addons_key();
 ...
 $install_addon=current_user_can( 'wzspbb_edit_addons' );
 ...
 <script type="text/javascript">
  jQuery(document).ready(function($){
     ...
     button.text('<?php _e('Wait...','woo-zoho-sync-pro') ?>');
  ...
  <div class="wrap" data-class="<?php echo $this->id ?>">
     <h2><?php _e('Manage Add-ons','woo-zoho-sync-pro'); ?></h2>
  <p><?php _e('You can activate, deactivate and delete add-ons here.','woo-zoho-sync-pro'); ?> </p>
  <?php
...
?></h2>
  <?php
    ...
    foreach($addons['addons'] as $slug=>$plugin){
...
$status=$this->plugin_status($plugin['class'],$slug);
 ...
 $refresh=admin_url( 'admin.php?page='.$this->id."&vx_action=get_addons");
    ?>
<div style="text-align: center; padding-top: 50px;">  <form method="post"><button name="vx_action" class="button button-hero" value="get_addons"><?php _e('Refresh Add-ons List','woo-zoho-sync-pro') ?></button></form>
...
if(class_exists('wzspbb_plugin_api') && !empty(wzspbb_plugin_api::$lics)  ){
  $users=array();
    if(empty(wzspbb_plugin_api::$updates['users']) ){
    wzspbb_plugin_api::$updates=get_option("cfx_plugin_updates",array());
    }
    if(!empty(wzspbb_plugin_api::$updates['users']) && is_array(wzspbb_plugin_api::$updates['users'])){
    foreach(wzspbb_plugin_api::$updates['users'] as $user){
        if(!empty($user['addons_access']) && !empty($user['user']) && !empty($user['plugins']) && is_array($user['plugins']) ){
            $skus=array_keys(wzspbb_plugin_api::$lics);
...
public function addon_box($plugin,$status,$install_addon){
    $status_msg=array(__('Not Installed','woo-zoho-sync-pro'),__('Active','woo-zoho-sync-pro'),__('Disabled','woo-zoho-sync-pro'));
  ...
  ?>
  <div class="vx_alert"><?php echo sprintf(__('This Plugin requires %s plugin','woo-zoho-sync-pro'),"<i>".$addon['name']."</i>") ?></div>
  <?php
      }}
...
if($status == 1 && !empty($addon_version) && version_compare($addon_version,$plugin['version'],"<") ){
     $new_version=true;
  ?>
  <div class="vx_alert"><?php echo sprintf(__('New Version %s is available.','woo-zoho-sync-pro'),"<i>".$plugin['version']."</i>") ?></div>
  <?php
 }
  ?>
  <div class="vx_box_head"><?php echo $plugin['name'] ?></div>
  <div class="vx_box_info"><?php _e('Updated :','woo-zoho-sync-pro'); ?> <i><?php echo $plugin['last_updated'] ?></i> &nbsp;&nbsp;&nbsp; <?php _e('Version :','woo-zoho-sync-pro'); ?>  <i><?php echo $plugin['version'] ?></i></div>
 ...
 <div class="vx_box_footer">
  ...
  ?>
  <a href="<?php echo $install ?>" class="button button-primary"><?php _e('Install','woo-zoho-sync-pro'); ?></a>
  <a href="<?php echo $plugin['package']; ?>" class="button button-primary"><?php _e('Download','woo-zoho-sync-pro'); ?></a>
  <?php
      }else  if($status== 2 && $install_addon ){
      $activate=$this->get_link('activate',$plugin['plugin']);
      $delete=$this->get_link('delete',$plugin['plugin']);
  ?>
    <a href="<?php echo $activate ?>" class="button button-primary vx_box_footer_link"><?php _e('Activate','woo-zoho-sync-pro'); ?></a>
     <a href="<?php echo $delete ?>" class="button"><?php _e("Delete",'woo-zoho-sync-pro'); ?></a>
  <?php
      }else if($status== 1 ){
if($install_addon){
 $deactivate=$this->get_link('deactivate',$plugin['plugin']);
  ?>
    <a href="<?php echo  $deactivate ?>" class="button button-primary vx_box_footer_link"><?php _e("Deactivate",'woo-zoho-sync-pro'); ?></a>
  <?php
  }
  global ${$plugin['class']};
  $cl=${$plugin['class']};
if(!is_object($cl) && class_exists($plugin['class'])){
 ...
 $settings_link=admin_url('admin.php?page='.$cl->page);
  ?>
      <a href="<?php echo  $settings_link ?>" class="button button-secondary"><?php _e("Go to Settings",'woo-zoho-sync-pro'); ?></a>
     <?php
 }
      }
...
?>
<a href="<?php echo $install ?>" class="button button-primary"><?php _e('Upgrade','woo-zoho-sync-pro'); ?></a>
    <?php
   }
  ?>
  </div>
...
public function manage_addons_action( $actions, $plugin_file ) {
        ...
        if(in_array($action,array('install-plugin','upgrade-plugin')) && $this->is_addon){
            $actions['plugins_page'] = '<a href="'.$this->get_link('settings').'">' . __('Back to Zoho Sync Pro Page', 'woo-zoho-sync-pro') . '</a>';
        }
...
public function manage_addons_ajax() {
    if(!current_user_can("wzspbb_edit_addons")){
    die(json_encode(array("status"=>"error","msg"=>__('Access Denied', 'woo-zoho-sync-pro'))));
  }
check_ajax_referer("vx_nonce","vx_nonce");
      ...
      $error="";
        if(empty($request)) {
        $error=__('Add-on name not specified.', 'woo-zoho-sync-pro');
        }
  if(!empty($error)){
...
if(!$result){
    $error= __('ERROR', 'woo-zoho-sync-pro');
}
                break;
        }
    ...
    $p_arr=explode("/",$plugin);
      if(!isset($plugins[$p_arr[0]]['class'])){
          $error= __('Plugin Not Found', 'woo-zoho-sync-pro');
      }
...
$wzspbb_addons=new wzspbb_addons();
endif;
